# -*- coding: utf-8 -*-
"""
Created on Thu May 16 14:58:10 2024
...
def dynamics(t, z_flat, alpha, beta, u, b, lattice_size, K, gamma):
    ...
    for x in range(lattice_size[0]):
        for y in range(lattice_size[1]):
            for j in range(4):
                ...
                term1_r = (alpha / (1 + z_i / K)) * ((beta + z_r * z_r) / (1 + z_r * z_r)) - z_r
                term1_i = gamma * (z_r - z_i)
...
lattice_size = (10, 10)  # Lattice size
...
z_real = np.random.uniform(0.0, 1.0, size=(lattice_size[0], lattice_size[1], 4))
z_imag = np.random.uniform(0.0, 1.0, size=(lattice_size[0], lattice_size[1], 4))
...
sol = solve_ivp(dynamics, [0, duration], z_init.flatten(), method='RK45', t_eval=t_eval, args=(alpha, beta, u, b, lattice_size, K, gamma))
...
#             plt.plot(sol.t[-250000:], np.real(sol.y[oscillator_index, -250000:]), label=f"Oscillator {i}")
#         plt.xlabel('Time')
#         plt.ylabel(f'Real(z)')
